...
export default function TradeDetailPage() {
  ...
  const rr = trade.risk_reward_ratio
    ? Number.parseFloat(trade.risk_reward_ratio)
    : null;
  ...
  return (
    <main
      style={{ minHeight: "100vh", background: "#0a0a0a", padding: "24px" }}
    >
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        >
          ...
          <div style={{ background: "#0a0a0a", padding: "24px" }}>
            ...
            {[
              { label: "ENTRY", value: trade.entry_price, color: "#e8e8e8" },
              { label: "EXIT", value: trade.exit_price, color: "#e8e8e8" },
              { label: "STOP LOSS", value: trade.stop_loss, color: "#ff3b3b" },
              {
                label: "TAKE PROFIT",
                value: trade.take_profit,
                color: "#00ff88",
              },
              { label: "LOT SIZE", value: trade.lot_size, color: "#888" },
            ].map((row) => (
            ...
            {[
              {
                ...
                value: trade.entry_time
                  ? format(parseISO(trade.entry_time), "dd MMM yyyy HH:mm")
                  : null,
              },
              {
                label: "EXIT",
                value: trade.exit_time
                  ? format(parseISO(trade.exit_time), "dd MMM yyyy HH:mm")
                  : null,
              },
              {
                label: "DURATION",
              ...
              },
              {
                ...
                value: trade.created_at
                  ? format(parseISO(trade.created_at), "dd MMM yyyy HH:mm")
                  : null,
              },
            ].map((row) => (
              <div key={row.label} style={rowStyle}>
        ...
        >
          ...
          >
            ...
            >
              {uploading ? "UPLOADING..." : screenshot ? "REPLACE " : "UPLOAD "}
              <input
                type="file"
                accept="image/*"
                style={{ display: "none" }}
